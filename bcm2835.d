...
uint * bcm2835_regbase(ubyte regbase)
{
    switch (regbase)
    {
        case BCM2835_REGBASE_ST:
            return cast(uint *)bcm2835_st;
        case BCM2835_REGBASE_GPIO:
            return cast(uint *)bcm2835_gpio;
        case BCM2835_REGBASE_PWM:
            return cast(uint *)bcm2835_pwm;
        case BCM2835_REGBASE_CLK:
            return cast(uint *)bcm2835_clk;
        case BCM2835_REGBASE_PADS:
            return cast(uint *)bcm2835_pads;
        case BCM2835_REGBASE_SPI0:
            return cast(uint *)bcm2835_spi0;
        case BCM2835_REGBASE_BSC0:
            return cast(uint *)bcm2835_bsc0;
        case BCM2835_REGBASE_BSC1:
            return cast(uint *)bcm2835_st;
    }
    return cast(uint *)MAP_FAILED;
}
...
uint bcm2835_peri_read(uint * paddr)
{
    if (DEBUG)
    {
        writefln("bcm2835_peri_read  paddr %08X", cast(uint)paddr);
        return 0;
    }
    else
    {
...
uint bcm2835_peri_read_nb(uint * paddr)
{
    if (DEBUG)
    {
        writefln("bcm2835_peri_read_nb  paddr %08X", cast(uint)paddr);
        return 0;
    }
    else
    {
...
void bcm2835_peri_write(uint * paddr, uint value)
{
    if (DEBUG)
    {
        writefln("bcm2835_peri_write paddr %08X, value %08X", cast(uint)paddr, value);
    }
    else
    {
        version(__arm__) {
...
void bcm2835_peri_write_nb(uint * paddr, uint value)
{
    if (DEBUG)
    {
        writefln("bcm2835_peri_write_nb paddr %08X, value %08X", cast(uint)paddr, value);
    }
    else
    {
        *paddr = value;
...
void bcm2835_gpio_fsel(ubyte pin, ubyte mode)
{
    ...
    uint * paddr = bcm2835_gpio + BCM2835_GPFSEL0/4 + (pin/10);
    ubyte  shift = (pin % 10) * 3;
    uint   mask = BCM2835_GPIO_FSEL_MASK << shift;
    uint   value = mode << shift;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPSET0/4 + pin/32;
    ubyte shift = pin % 32;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPCLR0/4 + pin/32;
    ubyte shift = pin % 32;
    bcm2835_peri_write(paddr, 1 << shift);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPSET0/4;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPCLR0/4;
    bcm2835_peri_write(paddr, mask);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPLEV0/4 + pin/32;
    ubyte shift = pin % 32;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPEDS0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = bcm2835_peri_read(paddr);
    return (value & (1 << shift)) ? HIGH : LOW;
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPREN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPFEN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
    bcm2835_peri_set_bits(paddr, value, value);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPHEN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
    bcm2835_peri_set_bits(paddr, value, value);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPLEN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
    bcm2835_peri_set_bits(paddr, value, value);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPAREN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
    bcm2835_peri_set_bits(paddr, value, value);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPAFEN0/4 + pin/32;
    ubyte shift = pin % 32;
    uint value = 1 << shift;
    bcm2835_peri_set_bits(paddr, value, value);
}
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPPUD/4;
...
{
    uint * paddr = bcm2835_gpio + BCM2835_GPPUDCLK0/4 + pin/32;
    ubyte shift = pin % 32;
...
{
    uint * paddr = bcm2835_pads + BCM2835_PADS_GPIO_0_27/4 + group;
...
void bcm2835_delayMicroseconds(ulong micros)
{
    ...
    if (DEBUG)
    {
        ...
        writefln("bcm2835_delayMicroseconds %lld", micros);
        return;
    }
...
/*void bcm2835_spi_setBitOrder(ubyte __attribute__((unused)) order)
{
...
{
    uint * paddr = bcm2835_spi0 + BCM2835_SPI0_CLK/4;
    bcm2835_peri_write(paddr, divider);
}

void bcm2835_spi_setDataMode(ubyte mode)
{
    uint * paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;
...
{
    uint * paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;
    uint * fifo = bcm2835_spi0 + BCM2835_SPI0_FIFO/4;
    uint ret;
...
void bcm2835_spi_writenb(char* tbuf, uint len)
{
    uint * paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;
    uint * fifo = bcm2835_spi0 + BCM2835_SPI0_FIFO/4;
    uint i;
    ...
    */
    ...
    while (bcm2835_peri_read(paddr) & BCM2835_SPI0_CS_RXD)
        cast(void)bcm2835_peri_read_nb(fifo);
    }
...
void bcm2835_i2c_begin()
{
    ...
    version(I2C_V1) {
        uint * paddr = bcm2835_bsc0 + BCM2835_BSC_DIV/4;
        ...
        uint * paddr = bcm2835_bsc1 + BCM2835_BSC_DIV/4;
    ...
    */
    i2c_byte_wait_us = (cast(float)cdiv / BCM2835_CORE_CLK_HZ) * 1000000 * 9;
}
...
void bcm2835_i2c_setSlaveAddress(ubyte addr)
{
    ...
    version(I2C_V1) {
        uint * paddr = bcm2835_bsc0 + BCM2835_BSC_A/4;
    } else {   
        uint * paddr = bcm2835_bsc1 + BCM2835_BSC_A/4;
    }
...
void bcm2835_i2c_setClockDivider(ushort divider)
{
    ...
    // 9 = Clocks per byte : 8 bits + ACK
    */
    i2c_byte_wait_us = (cast(float)divider / BCM2835_CORE_CLK_HZ) * 1000000 * 9;
}
...
void bcm2835_i2c_set_baudrate(uint baudrate)
{
    ...
    bcm2835_i2c_setClockDivider( cast(ushort)divider );
}
...
{
    version(I2C_V1) {
        uint * dlen    = bcm2835_bsc0 + BCM2835_BSC_DLEN/4;
        uint * fifo    = bcm2835_bsc0 + BCM2835_BSC_FIFO/4;
        uint * status  = bcm2835_bsc0 + BCM2835_BSC_S/4;
        uint * control = bcm2835_bsc0 + BCM2835_BSC_C/4;
    } else {
        uint * dlen    = bcm2835_bsc1 + BCM2835_BSC_DLEN/4;
        uint * fifo    = bcm2835_bsc1 + BCM2835_BSC_FIFO/4;
        uint * status  = bcm2835_bsc1 + BCM2835_BSC_S/4;
        uint * control = bcm2835_bsc1 + BCM2835_BSC_C/4;
    }   

    uint remaining = len;
    uint i = 0;
    ubyte reason = BCM2835_I2C_REASON_OK;
...
{
    ulong compare = offset_micros + micros;
...
*/
static void *mapmem(const string msg, size_t size, int fd, int off)
{
    void *map = mmap(NULL, size, (PROT_READ | PROT_WRITE), MAP_SHARED, fd, off);
    if (map == MAP_FAILED)
    stderr.writefln("bcm2835_init: %s mmap failed: %s", msg, strerror(errno));
    return map;
}
...
bool bcm2835_init()
{
    ...
    */
    try
    {
        fp = File(BMC2835_RPI2_DT_FILENAME , "rb");
        ubyte[4] buf;

        fp.seek(BMC2835_RPI2_DT_PERI_BASE_ADDRESS_OFFSET);

        if (fp.rawRead(buf).length == buf.length)
        {
            bcm2835_peripherals_base = cast(uint *)(buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3] << 0);
        }

        fp.seek(BMC2835_RPI2_DT_PERI_SIZE_OFFSET);
    ...
    {
        writeln("Could not open device tree. Error code: ", e.errno());
    }
...
bool bcm2835_close()
{
    if (DEBUG) { return true; } /* Success */

    unmapmem(cast(void*)&bcm2835_peripherals, bcm2835_peripherals_size);
    ...
    int main(int argc, char **argv)
    {
        ...
        */
        ...
        if (0) {
            ...
            {
                /* Read some data */
                ubyte value = bcm2835_gpio_lev(RPI_GPIO_P1_15);
                writefln("read from pin 15: %d", value);
            ...
            */
            while (1)
            {
                if (bcm2835_gpio_eds(RPI_GPIO_P1_15))
                {
                    ...
                    writeln("low event detect for pin 15");
                }
